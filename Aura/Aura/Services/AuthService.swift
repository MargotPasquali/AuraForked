...
/// Requête d'authentification envoyée au serveur.
struct AuthenticationRequest: Encodable {
    let username: String
    let password: String
}

/// Informations de transfert, par exemple, pour un transfert bancaire.
struct TransferInformation: Encodable {
    let recipient: String
    let amount: Float
}

/// Réponse d'authentification reçue du serveur.
...
/// Protocole pour le service d'authentification.
...
/// Enumération des erreurs possibles du service d'authentification.
...
/// Service d'authentification distant utilisant `NetworkManager` pour gérer les requêtes réseau.
